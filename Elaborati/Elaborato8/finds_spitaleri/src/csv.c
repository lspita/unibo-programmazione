...
void csv_at(const char *const filename, const bool header, const size_t row, char (*result)[CSV_COL_BUFFER_SIZE])
{
...
void csv_add(const char *const filename, const size_t cols, char (*datarow)[CSV_COL_BUFFER_SIZE])
{
    FILE *stream = secure_fopen(filename, "a");

    for (size_t i = 0; i < cols; i++)
    {
        fprintf(stream, "%s,", datarow[i]);
    }
    fputc('\n', stream);
