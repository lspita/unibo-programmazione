...
int Compare(char **StringToCompare, char **Comparator, int l, int NField, char **Title)
{
    ...
    for (int i = 0; i < l; i++)
    {
        ...
        {
            if (strcmp(Comparator[i], StringToCompare[i]) != 0 && strcmp(Comparator[i], "!") != 0 && i != NField)
            {
                if (ok == 1)
                {
                    printf("\nThe case you entered does not respect the general case for attribute-->");
                    ok = 0;
                }
                printf("  %s  ", Title[i]);
            }
    ...
    if (ok == 1)
        printf("\nThe case you entered respect the general case to have as attribute '%s'-->%s \n", Title[NField], Comparator[NField]);
    else
        printf("\n");
    return ok;
}
