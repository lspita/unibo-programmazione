...
void print_loan(const LOAN *const loan, const bool normalized) {
  printf("%s{"
         ...
         "\n\tnot_fully_paid: %d,"
         "\n}\n",
         normalized ? "(normalized) " : "", loan->credit_policy, loan->purpose,
         loan->int_rate, loan->installment, loan->log_annual_inc, loan->dti,
         loan->fico, loan->days_with_cr_line, loan->revol_bal, loan->revol_util,
         loan->inq_last_6mths, loan->delinq_2yrs, loan->pub_rec,
         loan->not_fully_paid);
}