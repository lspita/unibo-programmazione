...
double CharToDouble(char *char_array) {
  ...
  while (length > 0 &&
         (isspace(char_array[length - 1]) || char_array[length - 1] == '\n')) {
    char_array[--length] = '\0';
  }
  ...
  if (*endptr != '\0') {
    printf("Conversion of %s failed. Non-convertible character: %c", char_array,
           *endptr);
...
int partition(struct DataNode *arr, int low, int high) {
  ...
  for (int j = low; j <= high - 1; j++) {
    if (arr[j].Distance < pivot) { // Modifica la condizione da > a <
      i++;
      swap(&arr[i], &arr[j]);
    }
  }
