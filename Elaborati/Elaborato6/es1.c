...
int to_state(int row, int col, int max_col)
{
    return (row * max_col) + col;
}

void move_point(const struct Point *const current_pos,
                const struct Point *const movement,
                const int rows,
                const int cols,
                struct Point *out_pos)
{
    int new_row = (*current_pos).row + (*movement).row, new_col = (*current_pos).col + (*movement).col;

    (*out_pos) = (struct Point){
        new_row >= 0 ? (new_row >= rows ? new_row - rows : new_row) : rows + new_row,
        new_col >= 0 ? (new_col >= cols ? new_col - cols : new_col) : cols + new_col,
    };
}
...
void init()
{
    setlocale(LC_ALL, ""); // permit print of UTF-8 emoji and symbols ("" = portable to all locales)
    srand(time(NULL));
}
...
void random_in_border(const int rows, const int cols, struct Point *out_pos)
{
    ...
    const int random_col = random_row == 0 || random_row == rows - 1
...
void setup_field(const int rows, const int cols, enum StateType (*const field)[cols], struct Robot *const robot_ptr)
{
    // fill labirinth
    for (enum StateType *cell = &field[0][0]; cell <= &field[rows - 1][cols - 1]; cell++)
    {
...
void print_field(const int rows,
                 const int cols,
                 ...
                 const struct Robot *const robot_ptr)
{
    ...
    for (int row = 0; row < rows; row++)
    {
        ...
        for (int col = 0; col < cols; col++)
        {
            ...
            else if ((*robot_ptr).position.row == row && (*robot_ptr).position.col == col)
            {
                pin = (*robot_ptr).PIN;
            }
            else
...
void print_qtable(const int state, const int cols, const double (*const qtable)[cols])
{
...
void show_frame(const int qtable_state,
                const int qtable_cols,
                const double (*const qtable)[qtable_cols],
                const int field_rows,
...
enum Action best_action(const int state, const int n_actions, const double (*const qtable)[n_actions])
{
...
void update_qtable(const int current_state,
                   ...
                   double (*const qtable)[n_actions])
{
    ...
    qtable[current_state][action] +=
        LEARNING_RATE * (reward + (DISCOUNT_FUTURE * best_future) - qtable[current_state][action]);
}

void train_qtable(const long long iterations,
                  const int qtable_cols,
                  double (*const qtable)[qtable_cols],
                  const int field_rows,
                  const int field_cols,
                  const enum StateType (*const field)[field_cols],
                  const wchar_t *PINS,
                  ...
                  const struct Point *const MOVEMENTS)
{
    ...
    for (long long i = 1; i <= iterations; i++)
    {
        ...
        for (bool *c = &passed_cells[0][0]; c < &passed_cells[field_rows - 1][field_cols - 1]; c++)
        {
            *c = false;
        }
        ...
        while (!exit)
        {
            ...
            const int current_state = to_state(current_pos.row, current_pos.col, field_cols);
            const enum StateType current_statetype = field[current_pos.row][current_pos.col];

            int do_random = randint(1, 100);
            enum Action action = do_random <= random_chance ? randint(0, qtable_cols - 1)
                                                            : best_action(current_state, qtable_cols, qtable);

            struct Point new_pos;
            move_point(&current_pos, &MOVEMENTS[action], field_rows, field_cols, &new_pos);
            ...
            int reward = get_reward(new_statetype, passed_cells[new_pos.row][new_pos.col], stuck);
            update_qtable(current_state, action, reward, new_state, qtable_cols, qtable);

            exit = stuck || new_statetype == END || new_statetype == MINE;

            show_frame(current_state,
                       qtable_cols,
                       qtable,
                       field_rows,
                       field_cols,
...
void run_game(const int qtable_cols,
              const double (*const qtable)[qtable_cols],
              const int field_rows,
              const int field_cols,
              const enum StateType (*const field)[field_cols],
...
int main()
{
    ...
    for (double *cell = &qtable[0][0]; cell <= &qtable[QTABLE_ROWS - 1][QTABLE_COLS - 1]; cell++)
    {
        *cell = 0;
    }
    ...
    while (!exit)
    {
        ...
        const int current_state = to_state(robot.position.row, robot.position.col, field_cols);

        show_frame(current_state, QTABLE_COLS, qtable, field_rows, field_cols, field, PINS, NULL, &robot);
        ...
        if (run)
        {
            run_game(QTABLE_COLS, qtable, field_rows, field_cols, field, PINS, &robot, MOVEMENTS);
        }
        else
        {
            train_qtable(iterations, QTABLE_COLS, qtable, field_rows, field_cols, field, PINS, &robot, MOVEMENTS);
        }
    }
}