...
{
    char input;
    int spazi = 0, caratteriNuovaLinea = 0, caratteriLetti = 0, caratteriControllo = 0;
    int maiuscoli = 0, minuscoli = 0;
    int caratteriAlfaNumerici = 0, caratteriNumerici = 0, caratteriAlfabetici = 0, caratteriEsadecimali = 0,
        caratteriAscii = 0;
    ...
    do
    {
        ...
        caratteriLetti++; // conto ogni char in input per restituire il numero dei caratteri letti
        ...
        if (isprint(input)) // contatore caratteri stampabili (spazio incluso)
        {
            ...
            if (isgraph(input)) // contatore caratteri stampabili (spazio escluso)
            {
                ...
                if (isascii(input)) // contatore caratteri ascii
                {
                    ...
                    if (isalnum(input)) // contatore caratteri alfanumerici
                    {
                        ...
                        if (isxdigit(input)) // contatore caratteri esadecimali [A - F,a - f,0 - 9]
                        {
                            caratteriEsadecimali++;
                        }

                        if (isdigit(input))
                        { // contatore delle cifre numeriche
                            caratteriNumerici++;
                        }
                        else if (isalpha(input))
                        { // contatore caratteri dell'alfabeto
                            caratteriAlfabetici++;
                            if (isupper(input))
                            { // contatore caratteri maiuscoli
                                maiuscoli++;
                            }
                            else if (islower(input))
                            { // contatore caratteri minuscoli
                                minuscoli++;
                            }
                        }
                    }
                    else if (ispunct(input))
                    { // contatore punteggiatura
                        punteggiatura++;
                    }
                }
            }
            ...
            {
                if (isspace(input))
                { // contatore degli spazi
                    spazi++;
                }
                else if (iscntrl(input))
                { // contatore caratteri di controllo
                    caratteriControllo++;
                    if (input == '\n')
