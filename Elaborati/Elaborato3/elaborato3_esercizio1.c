...
void horizontalBorder(const int min, const int max, const unsigned int unit, bool showX)
{
    putchar('+');
...
// unit must be GCD of all continents' coordinates values and must hit 0 once in both axis
//(suggested: 1 or 2 with even x,y values)
void printGlobe(const struct Point *sw, const struct Point *ne, struct Continent *continents, int nContinents,
        ...
        for (int x = minX; x <= maxX; x += scale)
        {
            ...
            for (int i = 0; i < nContinents; i++)
            {
                ...
                else if (x > c.sw.x && x < c.ne.x)
                {
                    ...
                    else if (x >= middle(c.sw.x, c.ne.x) && y <= middle(c.sw.y, c.ne.y) &&
                             idPrinted[i] == false) // >= and idPrinted is needed because unit can be >1
                    {
...
bool getGuess(const struct Point *sw, const struct Point *ne, struct Continent *continents, int nContinents)
{
    const int minX = sw->x, minY = sw->y, maxX = ne->x, maxY = ne->y;
    ...
    {
        const struct Continent c = continents[i];
        if (generated.x >= c.sw.x && generated.x <= c.ne.x && generated.y >= c.sw.y && generated.y <= c.ne.y)
        {
            continentIndex = i;
            break;
        }
    ...
    if (continentIndex == -2)
    { // if no continent contains the point, it is in the ocean
        continentIndex = -1;
    }
